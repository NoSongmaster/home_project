...
class myconn(object):
    ...
    def rabbit(self,id_list):
        def call_print(ch, method, properties, body):
            callback_msg=json.loads(body.decode())
            print(callback_msg['msg'])
            ch.basic_ack(delivery_tag=method.delivery_tag)
            #结束等待
            channel.stop_consuming()
        connection = pika.BlockingConnection(pika.ConnectionParameters(host='192.168.0.102'))

        # channel.queue_declare(queue='salting')

        msg=input('请输入要执行的命令: ')
        for id_str in id_list:
            channel = connection.channel()
            channel.exchange_declare(exchange='saltstack', type='direct')
            callback_queue_name = str(uuid.uuid4())
            msg_dict={'cmd':msg,
                      'queuename':callback_queue_name
                      }
            print(id_str)
            channel.basic_publish(exchange='saltstack',
                                  routing_key=str(id_str),
                                  body=json.dumps(msg_dict))
            #创建一个临时队列
            channel.queue_declare(queue=callback_queue_name)
            #消费者，等待队列返回结果
            channel.basic_consume(call_print, queue=callback_queue_name)
            channel.start_consuming()

            #删除临时队列
            channel.queue_delete(callback_queue_name)
        # print(111111)
        connection.close()
        ...
        while True:
            ...
            for id_tt in id_list:
                ...
                if flag==0:
                    ...
                    break
            if flag!=0:
                cmd_str=input('请输入要操作的命令[salt/命令]:')
                if cmd_str.startswith('salt'):
                    self.rabbit(id_list)
                else:
